...
impl TaskManager {
    ...
    fn run_first_task(&self) -> ! {
        ...
        task0.first_sche_time = get_time_ms();
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            if inner.tasks[next].first_sche_time == 0 {
                inner.tasks[next].first_sche_time = get_time_ms();
            }
    ...
    fn first_sche_time(&self) -> usize {
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current].first_sche_time
    }
...
fn first_sche_time() -> usize {
    TASK_MANAGER.first_sche_time()
}

fn inc_syscall_num(syscall_id: usize) {
    TASK_MANAGER.inc_syscall_num(syscall_id);
}
fn get_syscall_num() -> [u32; MAX_SYSCALL_NUM] {
...
/// get fitst scheduled time
pub fn get_first_sche_time() -> usize {
    first_sche_time()
}
/// update syscall counter
pub fn update_syscall_counter(syscall_id: usize) {
    inc_syscall_num(syscall_id);
}
